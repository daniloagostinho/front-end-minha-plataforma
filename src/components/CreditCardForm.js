  ...
  const handleCardNumberChange = (e) => {
    ...
    if (bin.length === 6) {
      ...
      fetch('/api/payment-method', {
        ...
        body: JSON.stringify({ bin }), // Envia o bin no corpo da requisição
      })
        .then((response) => response.json())
        .then((data) => {
          if (data.paymentMethodId) {
            console.log('Payment Method ID:', data.paymentMethodId);
            setPaymentMethodId(data.paymentMethodId); // Armazena o paymentMethodId
          } else {
  ...
  return (
    <div className="w-full p-4">
      ...
      <form className="space-y-4">
        ...
        <button
          type="button"
          onClick={() => onConfirm(paymentMethodId)} // Passa o paymentMethodId ao confirmar
